...
export default function DonatePage() {
  ...
  return (
    <section id="donate" className="container py-8 dark:bg-transparent md:py-12 lg:py-24">
      {loading ? (
        <div className="flex items-center justify-center py-8">
          {/* Loader component instead of just text */}
          <Loader className="w-8 h-8" /> {/* Assuming a loader component */}
          <p className="text-muted-foreground text-lg ml-4">Loading...</p>
        </div>
      ) : error ? (
        <div className="flex flex-col items-center justify-center py-8">
          <p className="text-red-500 text-lg mb-4">{error}</p>
          <Button onClick={constructApiUrl}>Retry</Button>
        </div>
      ) : (
        <>
